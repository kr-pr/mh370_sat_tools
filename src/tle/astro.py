...
def jd(*time_ut1):
    ...
    jday = 367.0 * year - floor((7 * (year + floor((month + 9) / 12.0)))* 0.25) + \
        floor(275 * month / 9.0) + day + 1721013.5 + (second + minute * 60.0 + hour * 3600.0) / 86400.0
    return jday
...
def teme_to_ecef(r_teme, v_teme, x_p, y_p, theta, omega):
    ...
    xy = dot(rot_x, rot_y)
    r_ecef = r_teme.dot(rot_z).dot(xy)
    v_ecef = (v_teme - cross(array([0, 0, omega]), r_teme)).dot(rot_z).dot(xy)
